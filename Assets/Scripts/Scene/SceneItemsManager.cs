...
public class SceneItemsManager : SingletonMonobehaviour<SceneItemsManager>, ISaveable
{
    ...
    private void DestroySceneItems()
    {
        ...
        Item[] itemsScene = GameObject.FindObjectsOfType<Item>();

        // Loop through all scene items and destroy them
        for (int i = itemsScene.Length - 1; i > -1; i--)
        {
            Destroy(itemsScene[i].gameObject);
        }
    }
    ...
    public void ISaveableRestoreScene(string sceneName)
    {
        if (GameObjectSave.sceneData.TryGetValue(sceneName, out SceneSave sceneSave))
        {
            ...
            {
                // scene list items found - destrou existing items scene
                DestroySceneItems();
    ...
    public void ISaveableStoreScene(string sceneName)
    {
        ...
        // Creat list scene items dictionary in scene save and add to it
