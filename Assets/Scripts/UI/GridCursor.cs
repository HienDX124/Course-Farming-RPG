...
public class GridCursor : MonoBehaviour
{
        ...
        if (gridPropertyDetails != null)
        {
            ...
            switch (itemDetails.itemType)
            {
                ...
                case ItemType.Commondity:
                    if (!IsCursorValidForComodity(gridPropertyDetails))
                    {
                        SetCursorToInvalid();
                        return;
                    }
    ...
    /// <sumary>
    /// Test cursor validity for a comodity for the target gridPropertyDetails. Returns true if valid, false if invalid
    /// </sumary>
    private bool IsCursorValidForComodity(GridPropertyDetails gridPropertyDetails)
    {
    ...
    /// <sumary>
    // Sets the cursir as either valid or invalid for the tool for the target girdPropertyDetaiols. Returns true if valid or false if invalid
    /// </sumary>
        ...
        switch (itemDetails.itemType)
        {
            case ItemType.Hoeing_tool:
                if (gridPropertyDetails.isDiggable == true && gridPropertyDetails.daysSinceDug == -1)
                {
                    ...
                    // Loop through items found to see if any are reapable type - we are not going to let the player dig where there are reapable scene items
    ...
    {
        Vector3 worldPosition = mainCamera.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, -mainCamera.transform.position.z)); // z is how far the objects are in front of the camera - camera is at -10 so objects (-)-10 in front = 10
